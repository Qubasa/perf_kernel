...
impl<T: AsRef<[u8]>> Packet<T> {
    ...
    pub fn check_len(&self) -> Result<()> {
        ...
        if len < field::HEADER_END {
            // log::info!("len({}) < HEADER_END({})", len, field::HEADER_END);
            Err(Error::Truncated)
        } else {
            Ok(())
        }
...
impl<'a> Repr<'a> {
        ...
        T: AsRef<[u8]> + ?Sized,
    {
        ...
        match (packet.msg_type(), packet.msg_code()) {
            (Message::EchoRequest, code) => Ok(Repr::EchoRequest {
            ...
            (Message::EchoReply, code) => Ok(Repr::EchoReply {
                ident: packet.echo_ident(),
                seq_no: packet.echo_seq_no(),
                code: code,
                data: packet.data(),
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            Repr::EchoRequest {
                ...
                f,
                "ICMPv4 echo request id={} seq={} len={} code={:#x}",
                ident,
                seq_no,
                data.len(),
                code
            ...
            Repr::EchoReply {
                ...
                f,
                "ICMPv4 echo reply id={} seq={} len={} code{:#x}",
                ident,
                seq_no,
                data.len(),
                code
