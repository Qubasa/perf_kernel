...
impl<'a> IpPacket<'a> {
    ...
    pub(crate) fn emit_payload(
        ...
        match self {
            ...
            #[cfg(feature = "socket-udp")]
            IpPacket::Udp((_, udp_repr)) => udp_repr.emit(
    ...
    DeviceT: for<'d> Device<'d>,
{
    ...
    pub fn poll(&mut self, sockets: &mut SocketSet, timestamp: Instant) -> Result<bool> {
        ...
        loop {
            let processed_any = match self.socket_ingress(sockets, timestamp) {
        ...
        loop {
            ...
            rx_token.consume(timestamp, |frame| {
                match inner.device_capabilities.medium {
                    ...
                    Medium::Ip => {
                        inner
                            .process_ip(sockets, timestamp, &frame)
                            .map_err(|err| {
                                net_debug!("cannot process ingress packet: {}", err);
    ...
    fn socket_egress(&mut self, sockets: &mut SocketSet, timestamp: Instant) -> Result<bool> {
        ...
        for mut socket in sockets.iter_mut() {
            ...
            let socket_result = match *socket {
                #[cfg(feature = "socket-raw")]
                Socket::Raw(ref mut socket) => socket.dispatch(&_caps.checksum, |response| {
                ...
                Socket::Icmp(ref mut socket) => socket.dispatch(|response| match response {
                    #[cfg(feature = "proto-ipv4")]
                ...
                Socket::Udp(ref mut socket) => {
                    socket.dispatch(|response| respond!(IpPacket::Udp(response)))
                }
                #[cfg(feature = "socket-tcp")]
                Socket::Tcp(ref mut socket) => {
                    let ip_mtu = match _caps.medium {
                        #[cfg(feature = "medium-ethernet")]
                        Medium::Ethernet => {
                            _caps.max_transmission_unit - EthernetFrame::<&[u8]>::header_len()
                        }
                        #[cfg(feature = "medium-ip")]
...
impl<'a> InterfaceInner<'a> {
    ...
    fn process_icmpv4<'frame>(
        ...
        match icmp_repr {
            ...
            // FIXME: do something correct here?
            _ => {
                log::error!("icm_repr: Unrecognized");
        ...
        Tx: TxToken,
    {
        match packet {
            ...
            EthernetPacket::Ip(packet) => self.dispatch_ip(tx_token, timestamp, packet),
        }
    }
    ...
    fn has_neighbor(&self, addr: &IpAddress, timestamp: Instant) -> bool {
        match self.route(addr, timestamp) {
            Ok(_routed_addr) => match self.device_capabilities.medium {
                #[cfg(feature = "medium-ethernet")]
...
mod test {
    ...
    fn test_raw_socket_no_reply() {
        ...
        // Emit to frame
        let mut bytes = vec![0u8; ipv4_repr.buffer_len() + udp_repr.buffer_len()];
        let frame = {
            ipv4_repr.emit(
                &mut Ipv4Packet::new_unchecked(&mut bytes),
                &ChecksumCapabilities::default(),
