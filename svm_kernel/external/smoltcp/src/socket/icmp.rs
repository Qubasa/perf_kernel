    ...
    pub(crate) fn accepts(
        ...
        match (&self.endpoint, icmp_repr) {
            ...
            (&Endpoint::Ident(_), &IcmpRepr::Ipv4(Icmpv4Repr::EchoRequest { code, data, .. })) => {
                ...
                for &i in iter.clone() {
                    let s = u16::from_le_bytes(i);
                    sum = sum.wrapping_add(s);
                }
                if iter.remainder().len() > 0 {
                    sum = sum.wrapping_add(iter.remainder()[0] as u16);
                }
                let sum = ((sum & 0xFF) as u8).wrapping_add(((sum & (0xFF << 8)) >> 8) as u8);
                // log::info!(
                //     "sum({:#x}) == checksum({:#x}) = {}",
                //     sum,
                //     checksum,
...
mod test_ipv4 {
    ...
    fn test_send_dispatch() {
        ...
        assert_eq!(
            socket.dispatch(|(ip_repr, icmp_repr)| {
    ...
    fn test_recv_process() {
        ...
        assert_eq!(
            socket.process(&REMOTE_IPV4_REPR, &ECHOV4_REPR.into(), &checksum),
            Ok(())
        );
        assert!(socket.can_recv());
    ...
    fn test_accepts_udp() {
        ...
        assert_eq!(
            socket.process(&ip_repr, &icmp_repr.into(), &checksum),
            Ok(())
        );
        assert!(socket.can_recv());
...
mod test_ipv6 {
    ...
    fn test_recv_process() {
        ...
        assert_eq!(
            socket.process(&REMOTE_IPV6_REPR, &ECHOV6_REPR.into(), &checksum),
            Ok(())
        );
        assert!(socket.can_recv());
