...
use x86_64::structures::paging::{LegacyOffsetPageTable, PageTable};
use x86_64::VirtAddr;
...
/// Initialize a new LegacyOffsetPageTable.
///
/// This function is unsafe because the caller must guarantee that the
/// complete physical memory is mapped to virtual memory at the passed
/// `physical_memory_offset`. Also, this function must be only called once
/// to avoid aliasing `&mut` references (which is undefined behavior).
pub unsafe fn init(physical_memory_offset: VirtAddr) -> LegacyOffsetPageTable<'static> {
    let level_4_table = active_level_4_table(physical_memory_offset);
    LegacyOffsetPageTable::new(level_4_table, physical_memory_offset)
}

pub fn print_pagetable(mapper: &LegacyOffsetPageTable) {
...
pub unsafe fn map_and_read_phys<T: Copy>(
    mapper: &mut LegacyOffsetPageTable,
