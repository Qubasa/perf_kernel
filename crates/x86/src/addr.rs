...
///
/// This is a wrapper type around an `u32`, so it is always 8 bytes, even when compiled
...
#[derive(Clone, Copy, PartialEq, Eq, PartialOrd, Ord)]
#[repr(transparent)]
pub struct PhysAddr(u32); //TODO: PhyAddr can be up to 40 bits in legacy mode states the manual

/// A passed `u32` was not a valid virtual address.
///
...
/// address calculation.
#[derive(Debug)]
pub struct VirtAddrNotValid(u32);
...
impl VirtAddr {
    ...
    /// Converts the address to an `u32`.
    #[inline]
    ...
    // on this function. At least for 32- and 64-bit we know the `as u32` cast
    // doesn't truncate.
    #[inline]
    ...
    pub const fn p2_index(self) -> PageTableIndex {
        PageTableIndex::new_truncate((self.0 >> 22) as u16)
    }
}
...
/// A passed `u32` was not a valid physical address.
///
/// This means that bits 52 to 64 were not all null.
#[derive(Debug)]
pub struct PhysAddrNotValid(u32);
...
mod tests {
    ...
    pub fn test_align_up() {
        ...
        assert_eq!(align_up(0xffff_ffff, 1), 0xffff_ffff);
        // align 2
        assert_eq!(align_up(0, 2), 0);
        assert_eq!(align_up(1233, 2), 1234);
        assert_eq!(align_up(0xffff_fffe, 2), 0xffff_fffe);
        // address 0
        assert_eq!(align_up(0, 128), 0);
        assert_eq!(align_up(0, 1), 0);
        assert_eq!(align_up(0, 2), 0);
        assert_eq!(align_up(0, 0x8000_0000), 0);
    }
}
